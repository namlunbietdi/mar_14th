...
function mapMonitoringSnapshot(lastState, offlineThresholdInSeconds = 120) {
  ...
  return {
    ...
    routeNumber: lastState.routeNumber || lastState.dispatchOrderId?.routeId?.routeNumber || "",
    ...
    satellites: lastState.satellites,
    gpsFix: lastState.gpsFix,
    direction: lastState.direction,
    currentStopCode: lastState.currentStopCode,
    nextStopCode: lastState.nextStopCode,
    status: lastState.status,
    configVersion: lastState.configVersion,
    rssi: lastState.rssi,
...
function mapLocationLog(locationLog) {
  return {
    ...
    satellites: locationLog.satellites,
    gpsFix: locationLog.gpsFix,
    routeNumber: locationLog.routeNumber,
    direction: locationLog.direction,
    currentStopCode: locationLog.currentStopCode,
    nextStopCode: locationLog.nextStopCode,
    status: locationLog.status,
    configVersion: locationLog.configVersion,
    rssi: locationLog.rssi,
...
function mapDeviceEventLog(eventLog) {
  return {
    id: eventLog._id,
    deviceId: eventLog.deviceCode,
    vehicleId: eventLog.vehicleId?._id || eventLog.vehicleId || null,
    licensePlate: eventLog.vehicleId?.licensePlate || "",
    dispatchOrderId: eventLog.dispatchOrderId?._id || eventLog.dispatchOrderId || null,
    orderCode: eventLog.dispatchOrderId?.orderCode || "",
    routeId: eventLog.dispatchOrderId?.routeId?._id || eventLog.dispatchOrderId?.routeId || null,
    routeNumber: eventLog.routeNumber || eventLog.dispatchOrderId?.routeId?.routeNumber || "",
    routeName: eventLog.dispatchOrderId?.routeId?.routeName || "",
    eventType: eventLog.eventType,
    direction: eventLog.direction,
    stopCode: eventLog.stopCode,
